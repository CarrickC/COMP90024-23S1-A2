...
nodes = config.get('couchdb', 'nodes').split(',')

# try to connect to each node until successful
for node in nodes:
    try:
        url = f'http://{admin}:{password}@{node}:5984/'
...
api.add_resource(GlobalAutocorrelationSentiment, '/global_autocorrelation_sentiment',
                 '/global_autocorrelation_sentiment/<label>')
...
api.add_resource(LocalAutocorrelationSentiment, '/local_autocorrelation_sentiment',
                 '/local_autocorrelation_sentiment/<label>')
...
api.add_resource(SignificantLocalAutocorrelationSentiment, '/significant_local_autocorrelation_sentiment',
                 '/significant_local_autocorrelation_sentiment/<label>')
...
class TopWordCounts(Resource):
    def get(self):
        ...
        # retrieve all docs
        view_results = db.view('WordCounts/RawText', include_docs=True, startkey=[start_date, None],
                               endkey=[end_date, {}])
...
class TopWordCountsByAllSuburbs(Resource):
    def get(self):
        ...
        for suburb in suburbs:
            view_results = db.view('WordCounts/RawText', include_docs=True, startkey=[start_date, suburb],
                                   endkey=[end_date, suburb])
            ...
            words = [lemmatizer.lemmatize(word.lower()) for text in tweet_texts for word in word_tokenize(text) if
